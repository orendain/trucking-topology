...
import com.hortonworks.orendainx.trucking.topology.bolts.TruckAndTrafficMergeBolt
import com.hortonworks.orendainx.trucking.topology.nifi.MergedDataPacketBuilder
...
object TruckingTopology {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val config = if (args.nonEmpty) ConfigFactory.parseFile(File(args(0)).toJava) else ConfigFactory.load()
    ...
    StormSubmitter.submitTopology("truckingTopology", stormConfig, new TruckingTopology(config).buildTopology())
  }
}
