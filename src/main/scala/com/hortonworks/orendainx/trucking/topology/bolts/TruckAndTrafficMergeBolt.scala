...
//    .compatibility(SchemaCompatibility.BACKWARD).build()
  ...
  private def mergedTruckAndTrafficRecord(record: GenericRecord, truckData: TruckData, trafficData: TrafficData) = {
    record.put("eventTime", truckData.eventTime)
    record.put("truckId", truckData.truckId)
    record.put("driverId", truckData.driverId)
    record.put("driverName", truckData.driverName)
    record.put("routeId", truckData.routeId)
    record.put("routeName", truckData.routeName)
    record.put("latitude", truckData.latitude)
    record.put("longitude", truckData.longitude)
    record.put("speed", truckData.speed)
    record.put("eventType", truckData.eventType)
    record.put("congestionLevel", trafficData.congestionLevel)
    record
  }
